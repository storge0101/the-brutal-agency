...
export default async function handler(request: any, response: any) {
  ...
  const elapsedMs =
    typeof payload.elapsedMs === "number" ? payload.elapsedMs : Number(payload.elapsedMs || 0);
  ...
  if (message.length > 3000 || hasTooManyLinks(message)) {
    return response.status(400).json({ error: "Please simplify your message and try again." });
  }
